...
static void Cleanup(void *data) {
  ...
  if (state->device_block_times)
    cudaFree(state->device_block_times);
  if (state->device_block_smids)
    cudaFree(state->device_block_smids);
  if (state->device_buffer)
    cudaFree(state->device_buffer);
  // Free host memory.
  if (host_times->block_times)
    cudaFreeHost(host_times->block_times);
  if (host_times->block_smids)
    cudaFreeHost(host_times->block_smids);
...
static int AllocateMemory(TaskState *state) {
  ...
  if (!CheckCUDAError(
          cudaMalloc(&(state->device_block_times), block_times_size))) {
    return 0;
  }
  if (!CheckCUDAError(
          cudaMalloc(&(state->device_block_smids), block_smids_size))) {
    return 0;
  }
  if (!CheckCUDAError(
          cudaMalloc(&(state->device_buffer), sizeof(float) * 1000000))) {
    return 0;
  }
  // Allocate host memory.
  if (!CheckCUDAError(
          cudaMallocHost(&host_times->block_times, block_times_size))) {
    return 0;
  }
  if (!CheckCUDAError(
          cudaMallocHost(&host_times->block_smids, block_smids_size))) {
    return 0;
  }
  return 1;
}
...
static int initKernelConfigs(TaskState *state, char *info) {
  ...
  // If no mask is given, return from the function without parsing the remaining
...
static int CopyIn(void *data) { return 1; }

// Kernel that performs a simple computation
static __global__ void maxUtilizationKernel(float *output, int n,
...
static int Execute(void *data) {
  ...
  // launch the kernel.
  maxUtilizationKernel<<<state->block_count, state->thread_count, 0,
                         state->stream>>>(state->device_buffer, 200000,
...
static int CopyOut(void *data, TimingInformation *times) {
  ...
  if (!CheckCUDAError(cudaMemcpyAsync(host_times->block_times,
                                      state->device_block_times,
                                      block_times_count * sizeof(uint64_t),
                                      cudaMemcpyDeviceToHost, state->stream))) {
    return 0;
  }
  if (!CheckCUDAError(cudaMemcpyAsync(host_times->block_smids,
                                      state->device_block_smids,
                                      block_smids_count * sizeof(uint32_t),
                                      cudaMemcpyDeviceToHost, state->stream))) {
    return 0;
  }
  if (!CheckCUDAError(cudaStreamSynchronize(state->stream)))
...
static const char *GetName(void) { return "Busy Kernel"; }
