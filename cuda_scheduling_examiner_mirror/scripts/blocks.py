...
def plot_scenario(benchmarks, name, window_name, window_width, window_height,
    ...
    specific scenario."""
    if SAVE_AVIL:
        graph = BlockSMDisplay(window_name, get_block_intervals(name, benchmarks), window_width, window_height, start_time, end_time)
        canvasvg.saveall('output.svg', graph.canvas.canvas)
        print('SVG saved successfully.')
    else:
        print('SVG saving is not available.')
        return
