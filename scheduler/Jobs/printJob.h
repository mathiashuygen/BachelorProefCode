...
class PrintJob: public Job{
  ...
  private:
    ...
    struct printKernelLaunchInformation {
      Job*         jobPtr;       //pointer to the job, used for notifying the scheduler of job's completion. 
      ...
      printKernelLaunchInformation(Job* job, cudaStream_t stream, float* dptr, float* hptr, size_t sz, int id)
        : jobPtr(job), kernelStream(stream), devicePtr(dptr), hostPtr(hptr), size(sz), taskId(id){
        }

    };
    static void CUDART_CB printKernelCallback(cudaStream_t stream, cudaError_t status, void *data);
    static void addPrintKernelCallback(Job* job, cudaStream_t stream, float* dptr, float* hptr, size_t size, int id);
